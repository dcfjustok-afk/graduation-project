...
def append_after_last_paragraph(source, target):
    document = Document(target)
    if not document.paragraphs:
        raise ValueError("target document has no paragraphs")
...
if __name__ == "__main__":
    if len(sys.argv) not in (2, 3):
        raise SystemExit("usage: python scripts/append_markdown_after_last_paragraph_docx.py <markdown-path> [target-docx]")
    ...
    append_after_last_paragraph(source_path, target_path)
    print(target_path)
