...
def clone_children(children, target_document):
    body = target_document._body._element
    for child in children:
        body.append(deepcopy(child))


def find_heading_child_index(document, heading_text):
    ...
    for idx, child in enumerate(body_children):
        if child.tag.endswith("}p"):
            ...
            if paragraph_index < len(document.paragraphs):
                if document.paragraphs[paragraph_index].text.strip() == heading_text:
                    return idx
...
def insert_markdown_before(source, target, heading_text):
    ...
    body_children = [child for child in original._body._element if not child.tag.endswith("sectPr")]
    insert_at = find_heading_child_index(original, heading_text)
    clone_children(body_children[:insert_at], result)
...
if __name__ == "__main__":
    if len(sys.argv) not in (3, 4):
        raise SystemExit(
            "usage: python scripts/insert_markdown_before_heading_docx.py <markdown-path> <heading-text> [target-docx]",
        )
