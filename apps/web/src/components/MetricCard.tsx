...
const iconMap: Record<string, { icon: ReactNode; color: string }> = {
  '日志总量': { icon: <FileTextOutlined />, color: '#00d4ff' },
  '链上存证记录': { icon: <LinkOutlined />, color: '#00ff88' },
  '审计记录数': { icon: <SafetyCertificateOutlined />, color: '#7c3aed' },
  '活动告警数': { icon: <AlertOutlined />, color: '#ff3366' },
};

interface MetricCardProps extends OverviewCard {}

export function MetricCard({ title, value, suffix, trend }: MetricCardProps) {
  const mapping = iconMap[title] || { icon: <FileTextOutlined />, color: '#00d4ff' };
      ...
      <div className="metric-card__body">
        ...
        <div className="metric-card__info">
          ...
          <Statistic
            value={value}
            suffix={suffix}
            valueStyle={{ fontWeight: 700, color: '#e2e8f0', fontVariantNumeric: 'tabular-nums', fontSize: 28 }}
          />
