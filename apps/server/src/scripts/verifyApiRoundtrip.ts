...
async function requestItem<T>(baseUrl: string, path: string, init?: RequestInit) {
...
async function requestList<T>(baseUrl: string, path: string, init?: RequestInit) {
  const response = await fetch(`${baseUrl}${path}`, {
    ...init,
    headers: {
      "Content-Type": "application/json",
...
async function main() {
  ...
  try {
    const health = await requestItem<ServerHealthStatus>(baseUrl, "/api/health");
    assert.equal(typeof health.message, "string");

    const overview = await requestItem<ServerOverviewStats>(baseUrl, "/api/overview");
    assert.equal(typeof overview.data.totalLogs, "number");

    const taskId = `verify-api-task-${Date.now()}`;
    const payload: LogSubmitPayload = {
      taskId,
    ...
    const submit = await requestItem<LogSubmitResponseData>(baseUrl, "/api/logs", {
      method: "POST",
    ...
    const auditList = await requestList<ServerAuditRecord>(baseUrl, "/api/audits");
    assert.equal(Array.isArray(auditList.data), true);

    const alerts = await requestList<ServerAlertRecord>(baseUrl, "/api/alerts");
