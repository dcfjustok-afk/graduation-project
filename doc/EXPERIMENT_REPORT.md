...
本报告用于沉淀第 14 步“补测试和实验脚本”与第 15 步“篡改实验”的实际结果，内容均基于当前仓库中的真实脚本、真实数据库和本地 Hardhat 网络执行结果整理，可直接为论文实验章节提供素材。
...
- Agent 验证：增量读取、偏移量持久化、待重试队列验证成功
...
```json
{
  ...
  "minLatencyMs": 135.33,
  "maxLatencyMs": 352.78,
  "throughputPerSec": 5.77
}
```

结果分析：

- 在本地开发环境中，100 条日志提交全部成功，成功率为 100%。
- 平均响应时间约 173.2 ms，说明日志入库与链上写入闭环在单机环境下具备稳定性。
- 吞吐量约 5.77 条/秒，受本地链上交易确认和 SQLite 持久化共同影响。
...
```json
{
  ...
  "successCount": 5,
  "failureCount": 0,
  "avgLatencyMs": 9782.59,
  "maxLatencyMs": 10072.42,
  "avgProcessedCount": 136
}
```
...
- 单轮平均处理约 136 条日志记录。
- 平均时延约 9.78 s，说明当前批量审计逻辑更偏向“结果正确性优先”，后续若追求更高吞吐，可继续做并发控制和增量审计优化。
...
2. 在 100 条日志批量提交实验中，系统成功率达到 100%，平均响应时间为 173.2 ms。
3. 在 5 轮批量审计实验中，系统成功率达到 100%，平均每轮处理 136 条记录，平均耗时约 9.78 s。
4. 在篡改检测实验中，系统可稳定识别链下日志内容被修改的情况，并自动生成审计失败记录与异常告警。
5. 通过增加“合约代码存在性校验”和“按历史合约地址审计”两项修复，实验结果的可信度得到进一步提升。